...
export default function App() {
  ...
  // --------------------------------------------------------------------------
  // GEMINI API İLE FOTOĞRAFI ANALİZ ETME VE SORUYU ÇÖZME
  // --------------------------------------------------------------------------
...
Lütfen yanıtı SADECE ve kesinlikle aşağıdaki geçerli JSON formatında ver, markdown blokları veya ekstra açıklama metni ekleme:

{
  ...
  "teacherTip": "Öğrenciye sınav taktiği veya püf noktası"
      ...
      // JSON formatını dışarı taşıabilecek markdown işaretlerini temizleme
      rawText = rawText.replace(/```json/g, '').replace(/```/g, '').trim();
      
      // JSON başlangıç ve bitiş süslü parantezlerini güvene alma
      ...
      Alert.alert(
        ...
        'Gemini AI yanıtı işlenirken bir hata oluştu. Lütfen tekrar deneyiniz.\n\nDetay: ' + err.message
      );
    } finally {
      setLoading(false);
    }
